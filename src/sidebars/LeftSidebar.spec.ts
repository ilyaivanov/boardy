...
import { ClassName, cls, e, tIds } from "../infra";
...
const RenderTestSidebar = () => {
  const store: any = items.createSlapstukStore({
    ...items.initialState,
    items: initialItems,
  });
  items.setGlobalStore(store);
  return e(Provider, { store }, e(LeftSidebar, { onResize: jest.fn() }));
};

xdescribe("Slaptuk sidebar", () => {
  beforeEach(() => {
    jest.useFakeTimers();
...
//actions
const clickHideLeftSidebar = () =>
  fireEvent.click(get(tIds.leftSidebarToggler));
