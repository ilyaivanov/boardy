...
// import DragAvatar from "../../../slapstuk4/src/dragAndDrop/DragAvatar";
import { cls, colors, css, icons, cn } from "../infra";
  ...
  return (
    <div
      ...
      onClick={() => {
        if (items.isVideo(item))
          console.log("video clicked (TODO: actual play)");
    ...
    >
      ...
      {!isHome &&
        (isFocused
          ? icons.arrow({
              classNames: [cls.unfocusArrow, cls.icon],
              testId: "unfocus-" + item.id,
              onClick: (e) => {
                e.stopPropagation();
                // actions.unfocus();
              },
      ...
      >
        {icons.threeDotsVertical({
          classNames: [cls.rowMenuIcon, cls.icon],
          testId: "menu-" + item.id,
        })}
      </button>
    </div>
...
const viewItemIcon = (item: Item) => {
  return items.isVideo(item)
    ? icons.play({
        classNames: [cls.icon, cls.rowIcon, cls.sidebarVideoIcon],
        testId: "circle-" + item.id,
        onClick: (e) => {
          e.stopPropagation();
          // actions.playVideo(item.id);
        },
    ...
    ? icons.circle({
        classNames: [cls.icon, cls.rowIcon, cls.sidebarChannelIcon],
        testId: "circle-" + item.id,
        onClick: (e) => {
          e.stopPropagation();
    ...
    ? icons.circle({
        classNames: [cls.icon, cls.rowIcon, cls.sidebarPlaylistIcon],
        testId: "circle-" + item.id,
        onClick: (e) => {
          e.stopPropagation();
          // actions.focusItem(item.id);
    ...
    : icons.circle({
        classNames: [cls.icon, cls.rowIcon, cls.sidebarFolderIcon],
        testId: "circle-" + item.id,
        onClick: (e) => {
          e.stopPropagation();
          // actions.focusItem(item.id);
